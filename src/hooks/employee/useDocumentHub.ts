...
export function useDocumentHub() {
  ...
  // ── Upload — returns HubDocument so caller can navigate to viewer ─────────
  // CHANGED: was void, now returns HubDocument | null
  const uploadDocument = useCallback(async (
    ...
    try {
      ...
      // Refresh list in background — don't await so navigation is instant
      void fetchAll();
      return doc;
    } catch (e: unknown) {
      const err = e as { response?: { data?: { detail?: string } } };
  ...
  return {
    ...
    setActiveFilter,
    searchQuery,     setSearchQuery,
    uploadDocument,  // now returns HubDocument | null
    deleteDocument,
    refetch: fetchAll,
  };
}