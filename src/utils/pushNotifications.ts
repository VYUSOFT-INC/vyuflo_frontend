...
function urlBase64ToUint8Array(base64String: string): Uint8Array {
  ...
  const outputArray = new Uint8Array(new ArrayBuffer(rawBinary.length));
...
export async function subscribeToPush(): Promise<"ok" | "denied" | "unsupported" | "error"> {
  ...
  try {
    ...
    // Cast to BufferSource — TS 5.7+ types Uint8Array with ArrayBufferLike
    // (which includes SharedArrayBuffer) but PushManager expects plain
    // BufferSource. Runtime is unchanged.
    const appServerKey = urlBase64ToUint8Array(VAPID_PUBLIC_KEY) as unknown as BufferSource;
