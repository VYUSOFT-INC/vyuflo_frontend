...
// routes, so having two separate nav items pointing to identical content
// read as broken rather than just redundant. Login History already covers
// this ground for real.
...
export function SettingsSidebar({ onClose }: Props = {}) {
  ...
  // getFileUrl() is correct here — same reasoning as Sidebar.tsx: the backend
  // already returns a ready-to-use versioned avatar URL, not a raw storage key.
      ...
      <div className="px-[24px] pt-[24px] pb-[16px] shrink-0">
        ...
        <div className="flex items-center gap-[12px] mt-[16px] p-[12px] bg-[#f8fafc] rounded-[12px] border border-[#f1f5f9]">
          <div className="relative shrink-0">
            {/* FIXED: same fallback issue as Sidebar.tsx — onError used to
                just hide the broken <img>, leaving a blank gap instead of
