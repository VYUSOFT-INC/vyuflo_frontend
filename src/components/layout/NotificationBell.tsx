...
function NotifItem({ notif, onClick }: { notif: Notification; onClick: () => void }) {
  ...
  return (
    <button type="button" onClick={onClick}
      className={`w-full text-left px-[16px] py-[12px] flex items-start gap-[10px] transition
                  border-b border-[#f3f4f6] last:border-0 ${
        !notif.is_read ? "bg-[#fafbff] hover:bg-[#f0f5ff]" : "hover:bg-[#f9fafb]"
      }`}>
      <div className="w-[8px] h-[8px] rounded-full shrink-0 mt-[6px]"
        style={{ backgroundColor: notif.is_read ? "#e2e8f0" : dot }} />
      <div className="min-w-0 flex-1">
        <p className={`text-[13px] leading-[18px] ${
          !notif.is_read ? "font-semibold text-[#0f172a]" : "font-medium text-[#475569]"
        }`}>
          {notif.title}
        </p>
        <p className="text-[12px] text-[#94a3b8] mt-[2px] leading-[16px] line-clamp-1">{notif.body}</p>
        <div className="flex items-center gap-[6px] mt-[4px]">
          ...
          <span className="text-[11px] text-[#cbd5e1]">{timeAgo(notif.created_at)}</span>
        </div>
      </div>
      {!notif.is_read && (
    ...
    <>
      ...
      <div className="flex items-center justify-between px-[16px] py-[12px] border-b border-[#f1f5f9] shrink-0">
        <div className="flex items-center gap-[8px]">
          <h3 className="text-[14px] font-bold text-[#0f172a]">Notifications</h3>
          {unreadCount > 0 && (
            <span className="text-[10px] font-bold px-[6px] py-[1px] rounded-full text-white"
        ...
        {unreadCount > 0 && (
          <button onClick={onMarkAllRead}
            className="flex items-center gap-[4px] text-[11px] font-medium transition"
      ...
      <div className="flex-1 overflow-y-auto">
        {loading && recent.length === 0 ? (
          <div className="flex items-center justify-center py-[32px]">
            <svg className="w-5 h-5 animate-spin" style={{ color: "var(--theme-primary)" }}
          ...
          recent.map(n => (
            <NotifItem key={n.id} notif={n} onClick={() => onItemClick(n)} />
          ))
        )}
      </div>
      ...
      <button onClick={onViewAll}
        className="flex items-center justify-center gap-[4px] py-[12px] border-t border-[#f1f5f9]
                   text-[13px] font-medium hover:bg-[#f8fafc] transition shrink-0"
        style={{ color: "var(--theme-primary)" }}>
        View All Notifications <ChevronRight size={13} />
      </button>
    </>
...
export function NotificationBell({ className = "" }: Props) {
  ...
  const fullPagePath =
    role === "hr"        ? "/employer/notifications" :
    role === "attorney"  ? "/lawyer/notifications"   :
    role === "app_admin" ? "/admin/notifications"    :
                           "/notifications";

  const { notifications, unreadCount, markRead, markAllRead, loading, refetch } =
    useNotifications();
  ...
  const handleToggle = useCallback(() => {
    setOpen(v => { if (!v) refetch(); return !v; });
  }, [refetch]);
  ...
  const handleItemClick = useCallback((notif: Notification) => {
    markRead(notif.id);
    setOpen(false);
    navigate(getNotifRoute(notif) ?? fullPagePath);
  }, [markRead, navigate, fullPagePath]);
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {/* Bell button */}
      <button type="button" onClick={handleToggle} aria-label="Notifications"
        className="bg-white border border-[#e2e8f0] drop-shadow-[0px_1px_1px_rgba(0,0,0,0.05)]
                   flex items-center justify-center relative rounded-[10px] sm:rounded-[12px]
                   size-[34px] sm:size-[38px] lg:size-[40px]
                   hover:bg-[#f8fafc] transition-colors shrink-0">
        <Bell size={14} className="text-[#64748b]" />
        {unreadCount > 0 && (
          <span className="absolute -top-[4px] -right-[4px] min-w-[16px] h-[16px] px-[3px]
      ...
      {open && (
        <>
          {/* ── MOBILE: fixed, spans full width with 8px margin ── */}
          <div className="sm:hidden fixed z-[100] overflow-hidden flex flex-col bg-white
                          border border-[#e5e7eb] rounded-[14px]
                          shadow-[0_12px_32px_rgba(0,0,0,0.16)]"
            style={{
              top:    64,   // below mobile navbar (~56px) + 8px gap
              left:   8,
              right:  8,
              maxHeight: "75vh",
            }}>
          ...
          <div className="hidden sm:flex flex-col absolute right-0 top-[calc(100%+8px)]
                          w-[380px] bg-white border border-[#e5e7eb] rounded-[14px]
                          shadow-[0_12px_32px_rgba(0,0,0,0.12)]
                          z-50 overflow-hidden max-h-[480px]">
            <DropdownContent {...sharedProps} />
          </div>
        </>
      )}
