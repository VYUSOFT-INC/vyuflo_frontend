...
//   + data-tour="hr-kpi"      on KPI stat grid div
//   + data-tour="hr-actions"  on Quick Actions div
//   + data-tour="hr-widgets"  on widget grid wrapper div  (added alongside existing ref={gridRef})
//   + data-tour="hr-expiring" on expiring visas table div
//   + data-tour="hr-activity" on activity feed div
//   + <DashboardTour role="hr" user={user} /> at bottom of return
...
interface WidgetMeta { id: WidgetId; title: string; description: string; icon: ReactNode; defaultW: number; defaultH: number; minW: number; minH: number; }
...
function visaToken(code: string) {
  const m: Record<string, { bg: string; text: string }> = {
    'H-1B': { bg: '#eff6ff', text: '#1d4ed8' }, 'L-1': { bg: '#faf5ff', text: '#7e22ce' },
    'O-1':  { bg: '#fdf4ff', text: '#a21caf' }, 'TN':  { bg: '#f0fdfa', text: '#0f766e' },
    'E-3':  { bg: '#fdf2f8', text: '#be185d' },
  };
  return m[code] ?? { bg: '#eef2ff', text: '#4338ca' };
}
...
function WidgetPanel({ activeIds, onAdd, onRemove, open, onClose }: {
  ...
  if (!open) return null;
  return (
    <div ref={ref} className="absolute right-0 top-[48px] w-[360px] bg-white border border-[#e2e8f0] rounded-[14px] shadow-xl z-50 overflow-hidden">
      <div className="px-[16px] py-[12px] border-b border-[#f1f5f9] flex items-center justify-between">
        <div>
          <p className="text-[14px] font-semibold text-[#0f172a] tracking-[-0.5px]">Dashboard Widgets</p>
          <p className="text-[11px] text-[#94a3b8]">{activeIds.length} of {WIDGET_REGISTRY.length} active</p>
        </div>
        <button onClick={onClose} className="size-[28px] rounded-[8px] flex items-center justify-center text-[#94a3b8] hover:bg-[#f8fafc]"><X size={14} /></button>
      </div>
      <div className="max-h-[400px] overflow-y-auto p-[12px] flex flex-col gap-[8px]">
        {WIDGET_REGISTRY.map(w => {
          const active = activeIds.includes(w.id);
          return (
            <div key={w.id} className={`flex items-center gap-[10px] p-[10px] rounded-[10px] border transition ${active ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-[#f1f5f9] hover:border-[#e2e8f0]'}`}>
              <div className={`size-[34px] rounded-[8px] flex items-center justify-center shrink-0 ${active ? 'bg-indigo-100 text-indigo-600' : 'bg-[#f1f5f9] text-[#64748b]'}`}>{w.icon}</div>
              <div className="min-w-0 flex-1">
                <p className="text-[12px] font-semibold text-[#0f172a] tracking-[-0.4px]">{w.title}</p>
                <p className="text-[10px] text-[#94a3b8] tracking-[-0.3px] leading-[14px]">{w.description}</p>
              </div>
              <button onClick={() => active ? onRemove(w.id) : onAdd(w.id)}
                className={`size-[28px] rounded-[8px] flex items-center justify-center shrink-0 transition ${active ? 'bg-red-50 text-red-500 hover:bg-red-100' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}>
                {active ? <Minus size={14} /> : <Plus size={14} />}
              </button>
            </div>
          );
...
function StatCard({ label, value, delta, deltaSuffix, icon, iconBg, iconColor }: {
  ...
  return (
    <div className="bg-white border border-[#f1f5f9] rounded-[16px] p-[20px] shadow-[0px_1px_1px_rgba(0,0,0,0.05)]">
      <div className="flex items-start justify-between gap-[12px]">
        <span className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#64748b]">{label}</span>
        <div className="size-[36px] rounded-[10px] flex items-center justify-center shrink-0" style={{ backgroundColor: iconBg, color: iconColor }}>{icon}</div>
      </div>
      <p className="text-[28px] font-bold text-[#0f172a] tracking-[-0.5px] leading-[36px] mt-[8px]">{value.toLocaleString()}</p>
      <p className="text-[12px] tracking-[-0.5px] mt-[2px] inline-flex items-center gap-[4px]" style={{ color: trendColor }}>
        <TrendIcon size={13} />
...
export default function HRDashboard() {
  ...
  const addWidget = useCallback((id: WidgetId) => {
    ...
    const newItem: LayoutItem = { i: id, x: 0, y: Infinity, w: meta.defaultW, h: meta.defaultH, minW: meta.minW, minH: meta.minH };
    const newLayout = [...layouts.lg, newItem];
    setActiveWidgets(newIds); setLayouts({ lg: newLayout }); save(newIds, newLayout);
  }, [activeWidgets, layouts]);
  ...
  const onRowAction = (r: ExpiringVisa) => navigate(`/employer/employees/${r.employee_id}`);
        ...
        {error && !data ? (
          ...
          <div className="flex flex-col gap-[20px]">
            ...
            {isLoading && !data ? (
              ...
              <div data-tour="hr-kpi" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[16px]">
                <StatCard label="Total Employees"  value={data.stats.total_employees}  delta={data.stats.total_employees_delta ?? 0}  deltaSuffix="vs last quarter" icon={<Users size={18} />}         iconBg="#eef2ff" iconColor="#4f46e5" />
                <StatCard label="Active Visas"     value={data.stats.active_visas}     delta={data.stats.active_visas_delta ?? 0}     deltaSuffix="vs last quarter" icon={<ShieldCheck size={18} />}   iconBg="#f0fdf4" iconColor="#16a34a" />
                <StatCard label="Expiring Soon"    value={data.stats.expiring_soon}    delta={data.stats.expiring_soon_delta ?? 0}    deltaSuffix="needs attention" icon={<AlertTriangle size={18} />} iconBg="#fef2f2" iconColor="#dc2626" />
                <StatCard label="Pending Renewals" value={data.stats.pending_renewals} delta={data.stats.pending_renewals_delta ?? 0} deltaSuffix="in progress"     icon={<History size={18} />}       iconBg="#fff7ed" iconColor="#ea580c" />
              </div>
            )}

            {/* ═══ QUICK ACTIONS ═══ */}
            <div data-tour="hr-actions"
              className="bg-white border border-[#f1f5f9] rounded-[16px] p-[16px] shadow-[0px_1px_1px_rgba(0,0,0,0.05)] flex flex-wrap items-center gap-[12px]">
              <span className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#64748b] mr-[4px]">Quick Actions</span>
              <QuickAction primary icon={<BriefcaseBusiness size={15} />} label="New Case"         onClick={() => navigate('/employer/cases/new')} />
              <QuickAction primary icon={<UserPlus size={15} />}          label="Invite Employee"  onClick={() => navigate('/employer/invite')} />
              <QuickAction        icon={<Upload size={15} />}             label="Upload Documents" onClick={() => navigate('/employer/cases')} />
            ...
            {/* ═══ WIDGET GRID ═══ */}
            {data && activeWidgets.length > 0 && (
              <div data-tour="hr-widgets" ref={gridRef}>
                <Grid className="widget-grid" layout={layouts.lg} cols={12} rowHeight={70} width={gridWidth}
                  isDraggable isResizable draggableHandle=".widget-drag"
                  onLayoutChange={onLayoutChange} compactType="vertical" margin={[16, 16]}>
                  {activeWidgets.map(id => {
                    const meta = WIDGET_REGISTRY.find(w => w.id === id)!;
                    return (
                      <div key={id} className="bg-white border border-[#f1f5f9] rounded-[16px] shadow-[0px_1px_1px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col group">
                        <div className="widget-drag flex items-center gap-[8px] px-[16px] py-[10px] cursor-grab active:cursor-grabbing border-b border-[#f8fafc] shrink-0">
                          <GripVertical size={14} className="text-[#cbd5e1] group-hover:text-[#94a3b8] transition shrink-0" />
                          <p className="text-[13px] font-semibold text-[#0f172a] tracking-[-0.5px] flex-1 truncate">{meta.title}</p>
            ...
            {data && (
              <div data-tour="hr-expiring"
                className="bg-white border border-[#f1f5f9] rounded-[16px] overflow-hidden shadow-[0px_1px_1px_rgba(0,0,0,0.05)]">
                <div className="px-[20px] pt-[20px] pb-[14px] flex flex-col sm:flex-row sm:items-start justify-between gap-[12px]">
                  <div>
                    <h3 className="text-[16px] font-semibold text-[#0f172a] tracking-[-0.5px]">Visas Expiring Soon</h3>
                    <p className="text-[12px] text-[#64748b] mt-[2px]">Employees requiring immediate renewal action</p>
                ...
                <div className="grid grid-cols-[1.8fr_1fr_0.8fr_1fr_0.9fr_1.1fr_110px] gap-[16px] px-[20px] py-[10px] border-y border-[#f1f5f9] bg-[#f9fafb]">
                  {['Employee', 'Department', 'Visa Type', 'Expiry Date', 'Urgency', 'Status', 'Actions'].map((h, i) => (
                    <span key={h} className={`text-[10px] font-semibold uppercase tracking-[0.08em] text-[#94a3b8] ${i === 2 ? 'text-center' : i === 6 ? 'text-right' : ''}`}>{h}</span>
                  ))}
                </div>
                <div className="overflow-x-auto">
                  <div className="min-w-[900px]">
            ...
            {/* ═══ ACTIVITY FEED ═══ */}
            {data && (
              <div data-tour="hr-activity"
                className="bg-white border border-[#f1f5f9] rounded-[16px] p-[20px] shadow-[0px_1px_1px_rgba(0,0,0,0.05)]">
                <div className="flex items-center justify-between mb-[8px]">
                  <h3 className="text-[15px] font-semibold text-[#0f172a]">Recent Activity</h3>
                  <button onClick={() => navigate('/employer/notifications')}
                    className="text-[12px] font-medium text-indigo-600 hover:underline inline-flex items-center gap-[3px]">
