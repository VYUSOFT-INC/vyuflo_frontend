...
import type {
  HRCaseResponse, HRCaseStatus, HRCaseStage,
  HRCaseHistoryItem, HRApprovalStatus, AttorneyOption,
} from '../../types/hr/createCase.types';
import type { HRTaskResponse } from '../../types/hr/task.types';
import { UserAvatar } from '../../components/ui/UserAvatar';
...
// ─────────────────────────────────────────────────────────────────────────────
// ADD PARTICIPANT MODAL — assigns an attorney to the case (the case model
// has a single attorney_user_id, not a free-form participants list, so
// "adding a lawyer" here means picking/replacing the case's assigned
// attorney via the same field HRCreateCase's Step 4 sets).
// ─────────────────────────────────────────────────────────────────────────────

function AddParticipantModal({ open, onClose, onAssign }: {
  open: boolean;
  onClose: () => void;
  onAssign: (attorney: AttorneyOption) => Promise<void>;
}) {
  const [attorneys, setAttorneys] = useState<AttorneyOption[]>([]);
  const [loading,   setLoading]   = useState(false);
  const [search,    setSearch]    = useState('');
  const [selected,  setSelected]  = useState<AttorneyOption | null>(null);
  ...
  const filtered = attorneys.filter(a => {
    ...
    return !q || a.full_name.toLowerCase().includes(q) || (a.law_firm_name ?? '').toLowerCase().includes(q);
  });
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/30 z-50" onClick={onClose} />
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-[16px]">
        <div className="w-full max-w-[480px] bg-white rounded-[18px] border border-[#e2e8f0] shadow-2xl p-[24px] max-h-[85vh] flex flex-col">
          <div className="flex items-center justify-between mb-[16px]">
            <h3 className="text-[18px] font-semibold text-[#0f172a]">Add Participant</h3>
            <button onClick={onClose}><X size={18} className="text-[#94a3b8]" /></button>
          </div>

          <p className="text-[12px] text-[#64748b] mb-[12px]">
            Assign an immigration attorney to this case. They'll get access to review documents and generate letters.
          </p>
          ...
          <div className="relative mb-[12px]">
            <input
              ...
              className="w-full h-[38px] bg-white border border-[#e5e7eb] rounded-[8px] px-[10px] text-[13px] focus:outline-none focus:ring-2 focus:ring-indigo-200"
            />
          </div>

          <div className="flex-1 overflow-y-auto flex flex-col gap-[8px] mb-[16px] min-h-[120px]">
            {loading ? (
              [0, 1, 2].map(i => <div key={i} className="h-[64px] bg-[#f8fafc] rounded-[10px] animate-pulse" />)
            ) : filtered.length === 0 ? (
              <div className="text-center py-[24px] text-[13px] text-[#94a3b8]">
                {attorneys.length === 0 ? 'No attorneys available to assign.' : 'No attorneys match your search.'}
              </div>
            ) : (
              filtered.map(a => (
                <button
                  ...
                  className={`text-left flex items-center gap-[10px] p-[10px] rounded-[10px] border transition ${
                    selected?.user_id === a.user_id ? 'border-indigo-400 bg-indigo-50' : 'border-[#f1f5f9] hover:bg-[#f8fafc]'
                  }`}>
                  <div className="size-[36px] rounded-full flex items-center justify-center text-white text-[12px] font-bold shrink-0"
                       style={{ backgroundColor: avatarColor(a.full_name) }}>
                    {initials(a.full_name)}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-[13px] font-semibold text-[#111827] truncate">{a.full_name}</p>
                    <p className="text-[11px] text-[#64748b] truncate">
                      {a.law_firm_name ?? a.email} · {a.active_cases} active case{a.active_cases === 1 ? '' : 's'}
                    </p>
                  </div>
                  {!a.is_accepting && (
                    <span className="text-[10px] font-semibold text-[#a16207] bg-[#fef9c3] px-[6px] py-[2px] rounded-full shrink-0">
                      Not accepting
                    </span>
                  )}
                </button>
...
function Sidebar({ c, onApprove, onAddParticipant }: { c: HRCaseResponse; onApprove: () => void; onAddParticipant: () => void }) {
  const deadlines = [
    c.due_date ? { label: 'Target Submission', days: daysUntil(c.due_date) } : null,
  ].filter(Boolean) as Array<{ label: string; days: number | null }>;
    ...
    <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-[16px]">
      ...
      <div className="bg-white border border-[#f1f5f9] rounded-[14px] p-[20px] shadow-[0px_1px_1px_rgba(0,0,0,0.04)]">
        ...
        <div className="flex flex-col gap-[12px]">
          ...
          <button onClick={onAddParticipant} className="flex items-center gap-[6px] text-[12px] font-medium text-indigo-600 hover:underline mt-[2px]">
            <Plus size={12} /> Add Participant
          </button>
        </div>
      </div>
...
export default function HRCaseDetail() {
  ...
  const [showApprovalModal, setApprovalModal] = useState(false);
  const [showAddTaskModal,  setAddTaskModal]  = useState(false);
  const [showAddParticipantModal, setAddParticipantModal] = useState(false);
  ...
  // Add Participant — assigns the case's attorney_user_id. hr_case_routes.py's
  // PATCH /hr/cases/:id already lists attorney_user_id as an updatable field,
  // so this reuses that instead of a new endpoint. Setting the returned case
  // back into state is enough — Sidebar recomputes its participants list from
  // c.attorney, so the new attorney shows up immediately.
  const handleAssignAttorney = async (attorney: AttorneyOption) => {
    if (!applicationId) return;
    try {
      const updated = await createCaseApi.updateCase(applicationId, { attorney_user_id: attorney.user_id });
      setCase(updated);
      setAddParticipantModal(false);
      pushToast('success', 'Attorney added', `${attorney.full_name} now has access to this case.`);
    } catch {
      pushToast('error', 'Could not add attorney', 'Please try again.');
    }
  };
    ...
    <div className="flex flex-col h-full bg-[#f9fafb]" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <PageContent>
        <div className="flex flex-col gap-[0px]">
          ...
          {/* Main: sidebar + tab area */}
          <div className="flex flex-col lg:flex-row gap-[20px] items-start">
            <Sidebar c={c} onApprove={() => setApprovalModal(true)} onAddParticipant={() => setAddParticipantModal(true)} />
      ...
      <ApprovalModal open={showApprovalModal} current={c.hr_approval_status} onClose={() => setApprovalModal(false)} onSave={handleApproval} />
      <AddTaskModal open={showAddTaskModal} onClose={() => setAddTaskModal(false)} onSave={handleCreateTask} />
      <AddParticipantModal open={showAddParticipantModal} onClose={() => setAddParticipantModal(false)} onAssign={handleAssignAttorney} />
    </div>
  );
}