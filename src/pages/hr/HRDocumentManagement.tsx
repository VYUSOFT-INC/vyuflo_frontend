...
//   8. Preview — FIXED: inline modal instead of window.open (was blocked by
//      pop-up blockers since window.open ran AFTER an await, breaking the
//      trusted user-gesture chain). Matches the same blob-fetch pattern as
//      the working employee DocumentViewerPage, just rendered in a modal.
//   9. Preview modal Download link — FIXED: restored missing <a> opening tag
//      (was previously stripped out during an edit, breaking JSX parsing).
...
// ─────────────────────────────────────────────────────────────────────────────
// PREVIEW MODAL — NEW: replaces window.open (fixes pop-up blocking)
// ─────────────────────────────────────────────────────────────────────────────

function PreviewModal({ url, name, onClose }: { url: string | null; name: string; onClose: () => void }) {
  if (!url) return null;
  return (
    <div
    ...
    >
      ...
      >
        ...
        <iframe src={url} className="flex-1 w-full" title="Document preview" />
      </div>
    </div>
  );
}
...
export default function HRDocumentManagement({
  ...
  // NEW: preview modal state (replaces window.open)
  ...
  // ── Preview — FIXED: inline modal instead of window.open ──────────────────
  // window.open() was called AFTER an await, which breaks the trusted
  // user-gesture chain and gets blocked by pop-up blockers (Safari, Chrome
  // strict mode, etc). Rendering the blob inline avoids this entirely and
  // matches the working pattern already used by the employee document viewer.
  ...
  const handlePreview = async (doc: HRDocumentUIEntry) => {
    ...
    try {
      ...
      setPreviewName(doc.name || doc.document_type || 'document');
    } catch {
  ...
  // ── Download — same blob pattern but triggers download ────────────────────
  ...
  // ── Upload — uses correct document_type name ───────────────────────────────
  ...
  if (embedded) {
    return (
      <>
        {Content}
        <PreviewModal url={previewUrl} name={previewName} onClose={closePreview} />
        <RejectModal
