...
export default function I9SplitEditorPage() {
  ...
  // Which buffer are we currently waiting on to finish loading?
  const pendingIdxRef = useRef<0 | 1 | null>(null);
  // The currently visible URL (for Download button)
  ...
  // ── Regenerate the filled PDF (debounced) whenever `form` changes ──
  ...
  const regenerate = useCallback(async (values: I9FormData) => {
    ...
    try {
      ...
      // Write to the INACTIVE buffer — the visible iframe stays untouched
      // until this new URL finishes loading and onLoad flips z-index.
  ...
  // Debounced regen on every form change (500 ms — waits for user to pause)
  useEffect(() => {
    if (!templateRef.current) return;
    if (regenTimer.current) window.clearTimeout(regenTimer.current);
    regenTimer.current = window.setTimeout(() => regenerate(form), 500);
    return () => { if (regenTimer.current) window.clearTimeout(regenTimer.current); };
  }, [form, regenerate]);

  // Flip the visible iframe once the hidden one finishes loading the new PDF
          ...
          {pdfError ? (
            ...
            <>
              <iframe
                ...
                src={urlA ? `${urlA}#toolbar=1&navpanes=0&view=FitH` : 'about:blank'}
              ...
              <iframe
                ...
                src={urlB ? `${urlB}#toolbar=1&navpanes=0&view=FitH` : 'about:blank'}
