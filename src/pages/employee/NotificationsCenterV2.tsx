...
// // =============================================================================
// // Sound — plays a soft chime when a new notification arrives
// // =============================================================================
...
// // Shows browser notification popup (when app is in background/another tab)
// function showBrowserNotif(title: string, body: string, url: string) {
//   if (Notification.permission !== "granted") return;
//   const n = new Notification(title, {
//     body,
//     icon: "/logo192.png",
//     badge: "/logo192.png",
//     tag: "visaflow-notif",   // replaces previous if same tag
//   });
//   n.onclick = () => { window.focus(); window.history.pushState({}, "", url); n.close(); };
...
// const PREF_ITEMS: { label: string; sub: string; key: keyof UpdatePreferencesRequest }[] = [
//   { label: "Email Notifications",  sub: "Receive case & deadline updates by email",  key: "email_enabled"           },
//   { label: "Push Notifications",   sub: "Real-time alerts in your browser",          key: "push_enabled"            },
//   { label: "SMS Alerts",           sub: "Text messages for urgent actions only",     key: "sms_enabled"             },
//   { label: "Case Updates",         sub: "Status changes, attorney assignments",      key: "notify_case_updates"     },
//   { label: "Deadlines",            sub: "Reminders before visa deadlines",           key: "notify_deadlines"        },
//   { label: "Document Updates",     sub: "Approvals, rejections, missing docs",       key: "notify_document_updates" },
//   { label: "Immigration News",     sub: "Policy changes, USCIS updates",             key: "notify_news"             },
//   { label: "Security Alerts",      sub: "Login activity, account changes",           key: "notify_security_alerts"  },
// ];
...
// function NotifMenu({ notifId, isRead, onMarkRead, onDismiss }: {
//   notifId:     string;
//   isRead:      boolean;
...
//         className="text-[#d1d5db] hover:text-[#6b7280] transition p-[4px] rounded-lg hover:bg-[#f1f5f9]">
//         <MoreVertical size={14} />
//       </button>

//       {open && (
//         <div className="absolute right-0 top-[calc(100%+4px)] z-30 bg-white border border-[#e5e7eb]
//                         rounded-[10px] shadow-[0_4px_16px_rgba(0,0,0,0.10)] overflow-hidden w-[160px]">
...
//                 setOpen(false);
//               }}
//               className="w-full flex items-center gap-[8px] px-[12px] py-[9px] text-[12px]
//                          font-medium text-[#374151] hover:bg-[#f8fafc] transition text-left">
...
//             }}
//             className="w-full flex items-center gap-[8px] px-[12px] py-[9px] text-[12px]
//                        font-medium text-[#ef4444] hover:bg-[#fef2f2] transition text-left">
...
//           {/* Category icon */}
//           <div className="w-[40px] h-[40px] sm:w-[44px] sm:h-[44px] rounded-[10px] flex items-center
...
//               )}
//               {notif.actor_label && (
//                 <span className="text-[11px] font-medium text-[#6b7280]">{notif.actor_label}</span>
//               )}
//               <span className="text-[11px] text-[#9ca3af]">{timeAgo(notif.created_at)}</span>
//             </div>
//           </div>

//           {/* Right controls — arrow + 3-dot menu */}
//           <div className="flex items-center gap-[2px] flex-shrink-0 mt-[2px]"
//             onClick={e => e.stopPropagation()}>
//             {route && <ArrowRight size={13} className="text-[#d1d5db] mr-[2px]" />}
//             <NotifMenu
//               notifId={notif.id}
...
//                   if (route) navigate(route);
//                 }}
//                 className="text-[12px] sm:text-[13px] font-medium px-[14px] py-[6px] rounded-[8px]
...
//                 className="text-[12px] sm:text-[13px] font-medium px-[14px] py-[6px] rounded-[8px]
//                            border border-[#e5e7eb] text-[#374151] hover:bg-[#f9fafb] transition">
//                 {notif.cta_secondary_label}
//               </button>
//             )}
//           </div>
...
//     markRead, markAllRead, dismiss, loadMore, hasMore, refetch,
//   } = useNotifications({
...
//   // ── Init push SW on mount ─────────────────────────────────────────────────
//   useEffect(() => {
//     initPushNotifications(navigate);
//   }, [navigate]);

//   // ── Refetch on mount and tab change ───────────────────────────────────────
//   useEffect(() => { refetch(); }, []); // eslint-disable-line
//   useEffect(() => { refetch(); }, [activeTab]); // eslint-disable-line

//   // ── Detect new notifications → sound + browser popup ─────────────────────
//   useEffect(() => {
//     // Skip on first load (prevUnreadRef is null)
...
//       // New notification arrived since last poll
//       playNotificationSound();

//       // Browser popup — only when page is not focused
...
//             newest.title,
//             newest.body,
//             getNotifRoute(newest) ?? "/notifications",
//           );
//         }
//       }
//     }
...
//   // ── Push-aware preference toggle ──────────────────────────────────────────
//   const handlePrefToggle = useCallback(async (
//     key: keyof UpdatePreferencesRequest,
//     currentVal: boolean,
//   ) => {
...
//         if (perm !== "granted") return; // user denied — don't flip toggle
...
//           {/* Main grid */}
//           <div className="flex flex-col xl:flex-row gap-[20px] sm:gap-[24px] xl:items-start">
...
//                         className={`text-[12px] sm:text-[13px] font-medium px-[12px] sm:px-[14px]
//                                     py-[7px] sm:py-[8px] rounded-t-[6px] whitespace-nowrap transition
//                                     border-b-2 ${
//                           activeTab === tab.id
...
//                     <svg className="w-7 h-7 animate-spin" style={{ color: "var(--theme-primary)" }}
...
//                     <div className="w-[48px] h-[48px] bg-[#f1f5f9] rounded-full flex items-center
//                                     justify-center mb-[12px] text-[#9ca3af]">
//                       <Bell size={20} />
//                     </div>
...
//             {/* Right sidebar */}
//             <div className="flex flex-col gap-[16px] w-full xl:w-[300px] 2xl:w-[320px] xl:flex-shrink-0">
...
//                   <span className="font-medium text-[#6b7280]">what</span> you get notified about.
//                 </p>
//                 <div className="flex flex-col gap-[12px]">
//                   {visiblePrefs.map(p => {
//                     const val = prefs ? (prefs[p.key as keyof typeof prefs] as boolean ?? true) : true;
//                     return (
...
//                         </div>
//                         <Toggle checked={val} onChange={() => handlePrefToggle(p.key, val)} />
//                       </div>
//                     );
//                   })}
//                 </div>
//                 <button onClick={() => setShowAllPrefs(v => !v)}
...
//                 </button>
//                 {saving && <p className="text-[11px] text-[#9ca3af] mt-[8px]">Saving…</p>}
//               </div>

//               {/* Upcoming Deadlines */}
//               <div className="bg-white border border-[#e5e7eb] rounded-[12px] p-[20px]">
//                 <div className="flex items-center gap-[8px] mb-[14px]">
//                   <div className="w-[36px] h-[36px] bg-[#fff7ed] text-[#f97316] rounded-[8px]
...
//                       className={`bg-[#f8fafc] border border-[#f1f5f9] rounded-[10px] p-[12px] mb-[8px]
//                                   last:mb-0 ${route ? "cursor-pointer hover:bg-[#f1f5f9] transition-colors" : ""}`}>
...
//                       <span className="inline-block mt-[6px] text-[10px] font-bold px-[7px] py-[2px]
//                                        rounded-full bg-[#fff7ed] text-[#c2410c]">Deadline</span>
//                     </div>
//                   );
//                 })}
...
//               <div className="bg-white border border-[#e5e7eb] rounded-[12px] p-[20px]">
//                 <div className="flex items-center gap-[8px] mb-[14px]">
//                   <div className="w-[36px] h-[36px] bg-[#f5f3ff] text-[#7c3aed] rounded-[8px]
//                                   flex items-center justify-center flex-shrink-0">
//                     <Newspaper size={16} />
//                   </div>
...
//               <div className="bg-white border border-[#e5e7eb] rounded-[12px] p-[20px]">
//                 <div className="flex items-center gap-[8px] mb-[14px]">
//                   <div className="w-[36px] h-[36px] bg-[#f0fdf4] text-[#22c55e] rounded-[8px]
//                                   flex items-center justify-center flex-shrink-0">
//                     <TrendingUp size={16} />
//                   </div>
...
//                     <span className="text-[13px] font-bold" style={{ color: "var(--theme-primary)" }}>
//                       {caseNotifs.length}
//                     </span>
//                   </div>
...
//                   className="w-full mt-[14px] text-white text-[12px] font-medium rounded-[8px] py-[8px]
...
export default function NotificationsCenterV2() {
  ...
  // ── Push state — single source of truth, handles permission + subscribe ───
  const { toggle: togglePush, isGranted: pushGranted } = usePushNotifications();
  ...
  // ── Preference toggle — push routes through usePushNotifications hook ────
  // (that hook already calls updatePrefs internally for push_enabled,
  // so we only call updatePrefs directly for every OTHER preference key)
  const handlePrefToggle = useCallback(async (
    key: keyof UpdatePreferencesRequest,
    currentVal: boolean,
  ) => {
    if (key === "push_enabled") {
      await togglePush();   // hook handles permission request + subscribe + pref save
      return;
    }
    updatePrefs({ [key]: !currentVal });
  }, [togglePush, updatePrefs]);
          ...
          <div className="flex flex-col xl:flex-row gap-[20px] sm:gap-[24px] xl:items-start">
            ...
            <div className="flex flex-col gap-[16px] w-full xl:w-[300px] 2xl:w-[320px] xl:flex-shrink-0">
              ...
              <div className="bg-white border border-[#e5e7eb] rounded-[12px] p-[20px]">
                ...
                <div className="flex flex-col gap-[12px]">
                  {visiblePrefs.map(p => {
                    // Push row reflects actual browser permission + subscription state,
                    // not just the DB flag — avoids showing "on" when browser blocked it
                    const val = p.key === "push_enabled"
                      ? pushGranted
                      : prefs ? (prefs[p.key as keyof typeof prefs] as boolean ?? true) : true;
                    return (
                      <div key={p.key} className="flex items-center justify-between gap-[10px]">
                        <div className="min-w-0">
                          <p className="text-[12px] font-medium text-[#111827]">{p.label}</p>
