...
// src/pages/employee/DocumentUpload.tsx
//
// CHANGED: handleUpload() now navigates to DocumentViewer after upload
// instead of stopping at local state update.
// The viewer runs OCR, user reviews fields, then clicks Submit to confirm.
...
interface DocRow {
  ...
  description: string;
  status:      DocStatus;
  documentId?: string;   // real DB id once uploaded — used to open viewer
  file?: {
    name: string;
    size: string;
    date: string;
...
function DocRowItem({
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-start gap-[16px] md:gap-[24px] p-[16px] sm:p-[24px] w-full">
      ...
      {/* Right: spinner | file row | upload zone */}
      <div className="flex-1 min-w-0 w-full">
        {isUploading ? (
          /* Uploading spinner */
          <div className="flex items-center justify-center h-[72px] rounded-[12px] border border-[#e5e7eb] bg-[#f9fafb]">
          ...
          <div className={`flex items-center justify-between p-[12px] sm:p-[13px] rounded-[12px] border ${
            ...
            <div className="flex gap-[4px] sm:gap-[8px] items-center pl-[8px] sm:pl-[16px] shrink-0">
              {/* View — opens DocumentViewer with the real doc_id */}
              {doc.documentId && (
                <button
                  type="button"
                  onClick={() => onView(doc.documentId!)}
                  title="Review in Document Viewer"
                  className="flex items-center justify-center p-[6px] sm:px-[8px] sm:py-[6px] rounded-[8px] hover:bg-[#f3f4f6] transition">
                  <img src={imgEyeIcon} alt="View" className="w-[18px] h-[16px] object-contain" />
                </button>
              )}
...
export default function DocumentUpload() {
  ...
  // ── Upload → navigate to DocumentViewer for OCR review ──────────────────
  async function handleUpload(taskId: string, file: File) {
    if (!applicationId) return;
    setUploading(taskId);
    ...
    try {
      ...
      // Update local state so row reflects the upload immediately
      const fileEntry = {
        name: file.name,
        size: `${(file.size / 1024 / 1024).toFixed(1)} MB`,
        date: "Just now",
      ...
      setDocs(prev => prev.map(d =>
        d.id === taskId
          ? { ...d, status: "uploaded" as DocStatus, documentId: uploadedDoc.id, file: fileEntry }
          : d
      ));

      pushToast('success', 'Uploaded!', 'Opening document for review…');

      // Navigate to DocumentViewer for OCR review
      const returnUrl = encodeURIComponent(`/documents/upload?application_id=${applicationId}`);
      navigate(`/documents/viewer?doc_id=${uploadedDoc.id}&application_id=${applicationId}&return_url=${returnUrl}`);
    } catch (e) {
      console.error("Upload failed", e);
      pushToast('error', 'Upload failed', 'Please check the file and try again.');
    } finally {
      setUploading(null);
    }
  }

  // ── View button — open viewer for an already-uploaded doc ────────────────
  function handleView(documentId: string) {
    const returnUrl = encodeURIComponent(
      `/documents/upload?application_id=${applicationId}`
    );
    navigate(
      `/documents/viewer?doc_id=${documentId}&application_id=${applicationId}&return_url=${returnUrl}`
    );
  }

  function handleDelete(taskId: string) {
    setDocs(prev => prev.map(d =>
      d.id === taskId ? { ...d, status: "required" as DocStatus, file: undefined, documentId: undefined } : d
    ));
  }

  function handleSaveDraft() {
    setSaving(true);
    pushToast('info', 'Draft saved', 'Your progress has been saved.');
    const dest = applicationId ? `/applications/${applicationId}` : "/applications/list";
    setTimeout(() => { setSaving(false); navigate(dest); }, 800);
  }

  function handleSubmit() {
    const uploadedCount = docs.filter(d => d.file).length;
    if (uploadedCount === 0) {
      pushToast('warning', 'No documents uploaded', 'Please upload at least one document before submitting.');
      return;
    }
    const missingCount = docs.filter(d => !d.file).length;
    if (missingCount > 0) {
      pushToast('warning', `${missingCount} document${missingCount > 1 ? 's' : ''} missing`, 'You can still submit, but missing documents may delay your case.');
    }
    setSubmitting(true);
    const dest = applicationId ? `/applications/${applicationId}` : "/applications/list";
    setTimeout(() => {
      setSubmitting(false);
      pushToast('success', 'Submitted for review', 'Your documents have been sent to your case team.');
        ...
        <div className="flex flex-col gap-[24px] sm:gap-[32px] pb-[80px] max-w-[1200px] mx-auto">
          ...
          <div className="flex flex-col gap-[6px] sm:gap-[8px]">
            ...
            <p className="text-[#4b5563] text-[14px] sm:text-[16px] lg:text-[18px] font-normal leading-[24px] sm:leading-[28px]">
              Please provide the required documentation for your{" "}
              <span className="font-semibold text-[#111827]">{visaName}</span>{" "}
              application. After uploading, you'll review and confirm the extracted data before submission.
            </p>
          </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#e5e7eb]
                          ...
                          z-20">
            <p className="text-[#4b5563] text-[12px] sm:text-[14px] font-normal leading-[20px] hidden sm:block">
              Upload each document — you'll review extracted data before final submission.
            </p>
            <div className="flex gap-[10px] sm:gap-[16px] items-center w-full sm:w-auto justify-between sm:justify-end">
              <button type="button" onClick={handleSaveDraft} disabled={saving}
                className="border border-[#d1d5db] flex items-center justify-center
