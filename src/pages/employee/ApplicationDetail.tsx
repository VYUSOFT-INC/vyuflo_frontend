...
// // Moved to module scope (was previously re-declared inline inside handleUpload)
// // so TaskRow's HubPicker can also look up a task's category by name.
...
// // ─────────────────────────────────────────────────────────────────────────────
// // "FROM HUB" PICKER — lets the student reuse a document already in their
// // Document Hub instead of uploading the same file again from disk.
// // ─────────────────────────────────────────────────────────────────────────────

// // File-type visual treatment — colored icon tile + icon, same pattern as the
// // document type badges used elsewhere in this app (e.g. HRDocumentManagement).
...
//           {loading ? (
//             // Skeleton rows — consistent with the animate-pulse pattern used
//             // elsewhere in this app instead of a bare "Loading…" string.
//             [0, 1].map(i => (
...
// function TaskRow({ task, onView, onUpload, onReuse }: {
//   task: Task;
...
//   // Toggle state: which source tab is showing under this row ("upload" | "hub" | null = closed)
...
//               />
//               {/* Two entry points — "Upload" opens the local file dialog directly
//                   (fast path for the common case), "From Hub" opens the reuse picker. */}
//               <button
...
//       {/* Inline Hub picker — opens below the row when "From Hub" is clicked */}
//       {pickerTab === "hub" && (
...
// function OverviewTab({ app, tasksArr, onViewAllTasks, onViewTimeline, onUpload, onReuse, onView }: {
...
//                 <TaskRow key={task.id} task={task} onView={onView} onUpload={onUpload} onReuse={onReuse} />
//               ))}
...
// function TasksTab({ tasksArr, onView, onUpload, onReuse }: {
//   tasksArr: Task[];
//   onView: (docId: string) => void;
//   onUpload: (taskId: string, file: File) => void;
//   onReuse: (taskId: string, documentId: string) => void;
...
//           <div className="flex flex-col gap-[8px]">
//             {pending.map(t => <TaskRow key={t.id} task={t} onView={onView} onUpload={onUpload} onReuse={onReuse} />)}
//           </div>
//         </SectionCard>
//       )}
//       {completed.length > 0 && (
...
//           <div className="flex flex-col gap-[8px]">
//             {completed.map(t => <TaskRow key={t.id} task={t} onView={onView} onUpload={onUpload} onReuse={onReuse} />)}
//           </div>
//         </SectionCard>
//       )}
//       {tasksArr.length === 0 && (
...
// function DocumentsTab({ tasksArr, onView, onGoToTasks }: {
//   tasksArr: Task[];
//   onView: (docId: string) => void;
//   onGoToTasks: () => void;
// }) {
...
//   // Reuse a Document Hub file → link it to this task's application (no re-upload)
...
//     void taskId; // taskId kept in signature for symmetry with handleUpload / future optimistic updates
//   }

//   // Sidebar / Documents tab "Upload" shortcuts now just jump to the Tasks tab
//   // on THIS SAME PAGE, instead of navigating away to a separate upload page.
...
// src/pages/employee/ApplicationDetail.tsx
//
// CHANGED: Added delete functionality — trash icon next to "View" buttons
// in both the Documents tab and completed Task rows, with a confirmation
// modal before actually deleting.
...
function TaskRow({ task, onView, onUpload, onReuse, onDelete }: {
  task: Task;
  onView?: (docId: string) => void;
  onUpload?: (taskId: string, file: File) => void;
  onReuse?: (taskId: string, documentId: string) => void;
...
function OverviewTab({ app, tasksArr, onViewAllTasks, onViewTimeline, onUpload, onReuse, onView, onDelete }: {
  app: Record<string, unknown>;
  tasksArr: Task[];
  onViewAllTasks: () => void;
  onViewTimeline: () => void;
      ...
      <div className="bg-white border border-[#f1f5f9] rounded-[14px] shadow-[0px_1px_1px_rgba(0,0,0,0.04)] overflow-hidden">
        ...
        {pendingTasks.length > 0 && (
          <div className="px-[24px] py-[20px] border-b border-[#f8fafc]">
            ...
            <div className="flex flex-col gap-[8px]">
              {pendingTasks.slice(0, 3).map(task => (
                <TaskRow key={task.id} task={task} onView={onView} onUpload={onUpload} onReuse={onReuse} onDelete={onDelete} />
              ))}
              {pendingTasks.length > 3 && (
                <button onClick={onViewAllTasks}
                  className="text-[13px] font-medium text-indigo-600 hover:underline flex items-center gap-[3px] mt-[4px]">
...
function TasksTab({ tasksArr, onView, onUpload, onReuse, onDelete }: {
  tasksArr: Task[];
  onView: (docId: string) => void;
  onUpload: (taskId: string, file: File) => void;
  onReuse: (taskId: string, documentId: string) => void;
  ...
  return (
    <div className="flex flex-col gap-[16px]">
      {pending.length > 0 && (
        <SectionCard title={`Pending Tasks (${pending.length})`} icon={<AlertCircle size={14} className="text-[#ea580c]" />}>
          <div className="flex flex-col gap-[8px]">
            {pending.map(t => <TaskRow key={t.id} task={t} onView={onView} onUpload={onUpload} onReuse={onReuse} onDelete={onDelete} />)}
          </div>
        </SectionCard>
      )}
      {completed.length > 0 && (
        <SectionCard title={`Completed Tasks (${completed.length})`} icon={<CheckSquare size={14} className="text-[#15803d]" />}>
          <div className="flex flex-col gap-[8px]">
            {completed.map(t => <TaskRow key={t.id} task={t} onView={onView} onUpload={onUpload} onReuse={onReuse} onDelete={onDelete} />)}
          </div>
        </SectionCard>
      )}
      {tasksArr.length === 0 && (
...
function DocumentsTab({ tasksArr, onView, onGoToTasks, onDelete }: {
  tasksArr: Task[];
  onView: (docId: string) => void;
  onGoToTasks: () => void;
  onDelete: (docId: string, docName: string) => void;
...
export default function ApplicationDetail() {
  ...
  const {
    ...
    // ⚠️ ADJUST: if useApplicationTasks exports a refetch function under a
    // different name (e.g. `refresh`, `mutate`), rename this destructure
    // and the call inside handleConfirmDelete() below to match.
